...
const BlogSideMenu = ({ post }) => {
  const { user } = useUser();
  const { getToken } = useAuth();
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const { isLoading, error, data: savedBlogs } = useQuery({
    queryKey: ["savedBlogs"],
    queryFn: async () => {
      const token = await getToken();
      const response = await axios.get(`${import.meta.env.VITE_BASE_API_URL}/users/saved`, {
        headers: {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      return axios.delete(`${import.meta.env.VITE_BASE_API_URL}/blogs/delete/${post._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const token = await getToken(); // Await getToken
      return axios.patch(`${import.meta.env.VITE_BASE_API_URL}/users/save`, {
        postId: post._id,
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  if (error) return "Failed to fetch saved posts!";

  return (
    <div className="">
      <h1 className="mt-8 mb-4 text-sm font-medium">Actions</h1>
      {isLoading ? "Loading..." : error ? "Saved Post Fetching Failed!" : (
        <div className="flex items-center text-sm gap-2 py-2 cursor-pointer" onClick={handleSave}>
          <svg
          ...
          >
            <path
              ...
              stroke="black"
              strokeWidth="2"
              fill={saveMutation.isLoading ? (isSaved ? "none" : "black") : isSaved ? "black" : "none"}
            />
          </svg>
      ...
      {user && (post.user.username === user.username || isAdmin) && (
        <div onClick={handleDelete} className="flex items-center text-sm gap-2 py-2 cursor-pointer">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 48 48"
            fill="red"
